...
#### Phase 3: Platform Abstraction & POSIX Backend
- **Status**: Pending
- **Files**: `src/platform/platform.h`, `src/platform/platform_posix.c`, `src/platform/platform_linux.c`, `src/platform/platform_mac.c`, `src/core/main.c`
- **Scope**:
  - Implement `platform.h` interfaces for POSIX (termios, ioctl, signals, Linux/macOS sysinfo).
  - Implement `platform_write_output()` for POSIX using `write()`.
  - Wire `main.c` to orchestrate via `platform.h`.
- **Acceptance**: Linux and macOS compile cleanly via modular sources with zero regressions.
